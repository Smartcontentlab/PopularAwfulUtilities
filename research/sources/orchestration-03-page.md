...
Fetched: 2026-08-23T20:02:39.968Z

[Skip to content](https://openai.github.io/openai-agents-python/human_in_the_loop/#human-in-the-loop)
...
`needs_approval` is available on [`function_tool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.function_tool "function_tool"), [`Agent.as_tool`](https://openai.github.io/openai-agents-python/ref/agent/#agents.agent.Agent.as_tool "as_tool"), [`ShellTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.ShellTool "ShellTool            dataclass   "), and [`ApplyPatchTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.ApplyPatchTool "ApplyPatchTool            dataclass   "). Local MCP servers also support approvals through `require_approval` on [`MCPServerStdio`](https://openai.github.io/openai-agents-python/ref/mcp/server/#agents.mcp.server.MCPServerStdio "MCPServerStdio"), [`MCPServerSse`](https://openai.github.io/openai-agents-python/ref/mcp/server/#agents.mcp.server.MCPServerSse "MCPServerSse"), and [`MCPServerStreamableHttp`](https://openai.github.io/openai-agents-python/ref/mcp/server/#agents.mcp.server.MCPServerStreamableHttp "MCPServerStreamableHttp"). Hosted MCP servers support approvals via [`HostedMCPTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.HostedMCPTool "HostedMCPTool            dataclass   ") with `tool_config={"require_approval": "always"}` and an optional `on_approval_request` callback. Shell and apply\_patch tools accept an `on_approval` callback if you want to auto-approve or auto-reject without surfacing an interruption.
...
2. If an approval decision for that tool call is already stored in the [`RunContextWrapper`](https://openai.github.io/openai-agents-python/ref/run_context/#agents.run_context.RunContextWrapper "RunContextWrapper            dataclass   "), the runner proceeds without prompting. Per-call approvals are scoped to the specific call ID; pass `always_approve=True` or `always_reject=True` to persist the same decision for future calls to the same tool identity during the rest of the run.
3. If the approval rule requires approval and no decision for that tool call is stored, execution pauses, and `RunResult.interruptions` (or `RunResultStreaming.interruptions`) contains [`ToolApprovalItem`](https://openai.github.io/openai-agents-python/ref/items/#agents.items.ToolApprovalItem "ToolApprovalItem            dataclass   ") entries with details such as `agent.name`, `tool_name`, and `arguments`. This includes approvals raised after a handoff or inside nested `Agent.as_tool()` executions.
...
For approval requests from [`HostedMCPTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.HostedMCPTool "HostedMCPTool            dataclass   "), the Agents SDK identifies a sticky tool decision by the combination of `server_label` and tool name. An always-approve decision for `lookup_account` on one hosted MCP server does not approve a tool with the same name on another server. The Agents SDK persists an always-approve or always-reject decision only when the hosted MCP approval request includes both non-empty identity fields.
...
- Run-wide fallback: set [`RunConfig.tool_error_formatter`](https://openai.github.io/openai-agents-python/ref/run/#agents.run.RunConfig.tool_error_formatter "tool_error_formatter            class-attribute       instance-attribute   ") to control the default model-visible message for approval rejections across the whole run.
...
- Local [`ShellTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.ShellTool "ShellTool            dataclass   ") and [`ApplyPatchTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.ApplyPatchTool "ApplyPatchTool            dataclass   ") can use `on_approval` to approve or reject immediately in code.
- [`HostedMCPTool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.HostedMCPTool "HostedMCPTool            dataclass   ") can use `tool_config={"require_approval": "always"}` together with `on_approval_request` for the same kind of programmatic decision.
- Plain [`function_tool`](https://openai.github.io/openai-agents-python/ref/tool/#agents.tool.function_tool "function_tool") tools and [`Agent.as_tool()`](https://openai.github.io/openai-agents-python/ref/agent/#agents.agent.Agent.as_tool "as_tool") use the manual interruption flow on this page.

When these callbacks return a decision, the run continues without pausing for a human response. For Realtime and voice session APIs, see the approval flow in the [Realtime guide](https://openai.github.io/openai-agents-python/realtime/guide/).

## Streaming and sessions

The same interruption flow works in streaming runs. After a streamed run pauses, keep consuming [`RunResultStreaming.stream_events()`](https://openai.github.io/openai-agents-python/ref/result/#agents.result.RunResultStreaming.stream_events "stream_events            async   ") until the iterator finishes, inspect [`RunResultStreaming.interruptions`](https://openai.github.io/openai-agents-python/ref/result/#agents.result.RunResultStreaming.interruptions "interruptions            class-attribute       instance-attribute   "), resolve them, and resume with [`Runner.run_streamed(...)`](https://openai.github.io/openai-agents-python/ref/run/#agents.run.Runner.run_streamed "run_streamed            classmethod   ") if you want the resumed output to keep streaming. See [Streaming](https://openai.github.io/openai-agents-python/streaming/) for the streamed version of this pattern.

If you are also using a session, keep passing the same session instance when you resume from `RunState`, or pass another session object configured for the same session ID and backing store. The resumed turn is then appended to the same stored conversation history. See [Sessions](https://openai.github.io/openai-agents-python/sessions/) for the session lifecycle details.
...
- **Local shell and apply\_patch tools**: `ShellTool` and `ApplyPatchTool` also support `needs_approval`. Use `state.approve(interruption, always_approve=True)` or `state.reject(..., always_reject=True)` to cache the decision for future calls to that tool during the rest of the run. For automatic decisions, provide `on_approval` (see `examples/tools/shell.py`); for manual decisions, handle interruptions (see `examples/tools/shell_human_in_the_loop.py`). Hosted shell environments do not support `needs_approval` or `on_approval`; see the [tools guide](https://openai.github.io/openai-agents-python/tools/).
...
- **Realtime agents**: The realtime demo exposes WebSocket messages that approve or reject tool calls via `approve_tool_call` / `reject_tool_call` on the `RealtimeSession` (see `examples/realtime/app/server.py` for the server-side handlers and [Realtime guide](https://openai.github.io/openai-agents-python/realtime/guide/#tool-approvals) for the API surface).
